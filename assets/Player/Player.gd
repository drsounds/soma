...
onready var weapon_fire = [preload("res://assets/Player/Audio/Rifle_fire.wav"), preload("res://assets/Player/Audio/Pistol_fire.wav")]
onready var weapon_reload = [preload("res://assets/Player/Audio/Rifle_reload.wav"), preload("res://assets/Player/Audio/Pistol_reload.wav")]
onready var weapon_ray = $Camroot/h/v/pivot/Camera/ray

var weapons = ["Rifle", "Pistol"]
var current_weapon = -1
var fired_once = false
var is_motion_locked = false
...
export(bool) var is_embarked = false

export(bool) var is_player = true setget set_is_player, get_is_player

func set_is_player(value):
	is_player = value
	$Camroot/h/v/pivot/Camera.current = is_player

func set_medium(medium):
	self.medium = medium
	if medium == 'water':
		self.gravity = 0.1
		self.jump_magnitude = 0.1
		self.can_fly = true
	elif medium == 'space':
		self.gravity = 0.0
		self.jump_magnitude = 0.1
		...
		self.gravity = 28
		self.jump_magnitude = 15
		self.can_fly = false

func _on_passenger_disembark(car, passenger):
	add_child(passenger)
	passenger.transform = car.transform
	passenger.transform.origin.x -= 5
	passenger.rotation_degrees.x = 0
	passenger.rotation_degrees.y = 0
	passenger.rotation_degrees.z = 0
	
func _on_player_disembark(car, player):
	player.get_node('Camroot/h/v/pivot/Camera').current = true
	player.is_embarked = false
...
func _input(event):
	if not is_player or is_embarked:
		return
	_process_event(event)
...
func _process_event(event):
	...
	if event.is_action_pressed("use"):
		...
		print(parent)
		if parent != null:
			for node in getallnodes(parent):
				print(node)
				if node is Spatial:
